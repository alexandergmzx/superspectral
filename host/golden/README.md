...
| `pitch_method` (schema name: `analyses.pitch.method`) | `raw` (classic autocorrelation, `To Pitch (ac)`) or `filtered` (`To Pitch (filtered ac)`) | must be stated per golden set; `cc` (cross-correlation) only if a set explicitly needs it |

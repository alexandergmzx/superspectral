...
// Bring-up order per docs/hw/twatch-s3-pins.md: console -> I2C -> AXP2101 chip-ID -> ALDO3
// (display+touch)
...
#define PIN_LCD_BL \
    45 // VDD_SPI strap neutralised on this unit: VDD_SPI_FORCE=1 (efuse-baseline.json)
_Static_assert(PIN_I2C0_SDA != 19 && PIN_I2C0_SCL != 20 && PIN_LCD_DC != 19 && PIN_LCD_BL != 20,
               "GPIO19/20 are USB-Serial-JTAG");

// ---- AXP2101 (XPowersLib REG/AXP2101Constants.h, MIT) ----
#define AXP_ADDR    0x34
#define AXP_IC_TYPE 0x03 // reads 0x4A
#define AXP_CHIP_ID 0x4A
#define AXP_LDO_ONOFF_CTRL0 \
    0x90 // b0 ALDO1 b1 ALDO2 b2 ALDO3 b3 ALDO4 b4 BLDO1 b5 BLDO2 b6 CPUSLDO b7 DLDO1
#define AXP_ALDO2_VOL 0x93 // low 5 bits = (mV-500)/100
#define AXP_ALDO3_VOL 0x94

static i2c_master_dev_handle_t axp;

static esp_err_t axp_rd(uint8_t reg, uint8_t *v)
{
    return i2c_master_transmit_receive(axp, &reg, 1, v, 1, 100);
}
static esp_err_t axp_wr(uint8_t reg, uint8_t v)
{
    uint8_t b[2] = {reg, v};
    return i2c_master_transmit(axp, b, 2, 100);
}
static esp_err_t axp_set_ldo_mv(uint8_t reg, int mv)
{
    uint8_t cur;
    ESP_RETURN_ON_ERROR(axp_rd(reg, &cur), TAG, "rd");
    return axp_wr(reg, (uint8_t) ((cur & 0xE0) | (uint8_t) ((mv - 500) / 100)));
}
static esp_err_t axp_enable_bit(uint8_t bit)
{
    uint8_t cur;
    ESP_RETURN_ON_ERROR(axp_rd(AXP_LDO_ONOFF_CTRL0, &cur), TAG, "rd");
...
void app_main(void)
{
    ...
    esp_ota_img_states_t st = ESP_OTA_IMG_UNDEFINED; // stays UNDEFINED when no otadata record maps
                                                     // to this slot (after a rollback)
    (void) esp_ota_get_state_partition(run, &st);
    ESP_LOGI(TAG, "idf %s, lvgl %d.%d.%d, running %s @0x%lx state %d reset %d",
             esp_get_idf_version(), LVGL_VERSION_MAJOR, LVGL_VERSION_MINOR, LVGL_VERSION_PATCH,
             run->label, (unsigned long) run->address, (int) st, (int) esp_reset_reason());

    // ---- I2C0: PMU, IMU, RTC, haptics ----
    i2c_master_bus_handle_t bus0, bus1;
    i2c_master_bus_config_t b0 = {.i2c_port = 0,
    ...
    i2c_device_config_t dc = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7, .device_address = AXP_ADDR, .scl_speed_hz = 100000};
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus0, &dc, &axp));
    uint8_t id = 0;
    ESP_ERROR_CHECK(axp_rd(AXP_IC_TYPE, &id));
    ESP_LOGI(TAG, "AXP2101 IC_TYPE=0x%02X (%s)", id, id == AXP_CHIP_ID ? "ok" : "UNEXPECTED");
    if (id != AXP_CHIP_ID) {
        ESP_LOGE(TAG, "not an AXP2101 - stopping before touching rails");
        for (;;)
            vTaskDelay(1000);
    }
    uint8_t ldo;
    axp_rd(AXP_LDO_ONOFF_CTRL0, &ldo);
    ESP_LOGI(TAG, "LDO_ONOFF_CTRL0 before: 0x%02X", ldo);
    ...
    axp_rd(AXP_LDO_ONOFF_CTRL0, &ldo);
    ESP_LOGI(TAG, "LDO_ONOFF_CTRL0 after ALDO3: 0x%02X", ldo);

    // ---- I2C1: touch (FT6336U @0x38) should now answer ----
    i2c_master_bus_config_t b1 = {.i2c_port = 1,
    ...
    spi_bus_config_t spi = {.sclk_io_num = PIN_LCD_SCK,
    ...
    esp_lcd_panel_io_spi_config_t ioc = {.dc_gpio_num = PIN_LCD_DC,
    ...
    esp_lcd_panel_dev_config_t pc = {.reset_gpio_num = GPIO_NUM_NC,
    ...
    for (int i = 0; i < 1024; i++) {
        x[2 * i] = (i % 64) < 32 ? 1.0f : -1.0f;
        x[2 * i + 1] = 0;
    }
    dsps_fft2r_fc32(x, 1024);
    dsps_bit_rev2r_fc32(x, 1024);
    ...
    for (int b = 0; b < 48; b++) {
        float mag = 0;
        for (int k = b * 2; k < b * 2 + 2 && k < 512; k++) {
            float re = x[2 * k], im = x[2 * k + 1];
            float m = re * re + im * im;
            if (m > mag)
                mag = m;
        }
        int h = (int) (mag > 1 ? (10.0f * __builtin_log10f(mag)) * 2.0f : 0);
        if (h > 110)
            h = 110;
        if (h < 2)
            h = 2;
    ...
    ledc_timer_config_t lt = {.speed_mode = LEDC_LOW_SPEED_MODE,
                              .duty_resolution = LEDC_TIMER_8_BIT,
                              .timer_num = LEDC_TIMER_0,
                              .freq_hz = 5000,
                              .clk_cfg = LEDC_AUTO_CLK};
    ESP_ERROR_CHECK(ledc_timer_config(&lt));
    ledc_channel_config_t lch = {.gpio_num = PIN_LCD_BL,
    ...
    for (int d = 0; d <= 160; d += 8) {
        ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0, (uint32_t) d);
    ...
    if (st == ESP_OTA_IMG_PENDING_VERIFY) {
        ...
        ESP_LOGI(TAG, "marked valid");
    }
    (void) i2s_channel_init_pdm_rx_mode;
    (void) esp_vfs_littlefs_register;
    (void) esp_lcd_touch_new_i2c_ft5x06;
    ESP_LOGI(TAG, "GATE_STAGE2B_FRAME_SENT");
    for (int i = 0;; i++) {
        vTaskDelay(pdMS_TO_TICKS(5000));
        ESP_LOGI(TAG, "alive %d", i);
    }
}

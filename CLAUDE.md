...
**Super Spectral** — a wrist-worn singing-voice spectral analyzer on the LilyGO T-Watch S3 (ESP32-S3-R8, one Knowles SPM1423HM4H-B PDM microphone, 240×240 ST7789V3 display). The watch is the **live-capture and real-time-display front end** of a preset-driven analyzer; a Linux host does the offline science on recorded takes. That host also carries the founding research document's web application — a browser-native live analyzer and an offline-compare UI — as **its own** instrument ([ADR 0021](docs/adr/0021-host-web-application.md)). The watch carries forward the six Spectroid-style presets of the founding research document, [`docs/research/00-linux-analyzer-architecture-and-build-guide.md`](docs/research/00-linux-analyzer-architecture-and-build-guide.md).
...
| **Host — offline path + web application** (`host/`, Linux, GPL-3.0-or-later) | *Offline (Python):* Praat-grade formants (parselmouth), LTAS/SPR over whole takes, H1–H2, DTW alignment, Demucs stem separation, golden-file generation. *Web application ([ADR 0021](docs/adr/0021-host-web-application.md)):* front end `host/web/` (Vite + TypeScript) over a FastAPI backend `host/src/spectral_host/web/` — browser-native live spectrum, waterfall and f0 / ring / F1–F2 overlays from **the host machine's own microphone**, the six presets served byte-identical from `protocols/presets/`, an injection mode for the Tier-0 WAVs, and the offline-compare UI. Never linked into firmware, never in the watch's live loop, **no live link between watch and host**; its latency and refresh are **measured, never claimed**. |
...
The phase goal is a **complete, citable documentation base and a reproducible ESP-IDF environment before any feature firmware exists**. The roadmap ([`docs/roadmap/documentation-roadmap.md`](docs/roadmap/documentation-roadmap.md)) now runs three tracks — D (documentation/acquisition: D0–D6), E (environment: E0–E2) and W (the host web application: W0–W4, [ADR 0021](docs/adr/0021-host-web-application.md)). This pass delivers D0 (constitution), D1 (acquisition list) and E0 (environment specification); the rest are follow-up sessions because they change system state, touch the watch, or need the user present.
...
| [`host/`](host/) | Linux companion — **GPL-3.0-or-later, own `LICENSE`**: offline analysis, the Praat golden-file generator, and the web application ([ADR 0021](docs/adr/0021-host-web-application.md)) — front end `host/web/`, backend package `host/src/spectral_host/web/` |
...
**Deliberate, sole exception:** GPL-licensed companion code lives in [`host/`](host/) — the Linux offline-analysis path and the Praat/parselmouth golden-file generator under [`host/golden/`](host/golden/). It imports parselmouth (GPLv3) in-process, so it cannot be Apache-2.0, and the licence boundary must be a **directory boundary** that a reviewer can see at a glance. Nothing under `host/` is imported by anything outside `host/`; nothing outside `host/` is imported by `host/` except through files on disk (takes, manifests, golden vectors). The **TypeScript** under `host/web/` is GPL-3.0-or-later for exactly the same reason — it is the user interface of that same program, served by that same backend ([ADR 0021](docs/adr/0021-host-web-application.md) decision 4). Do not add Python outside `python-scripts/` on the strength of this precedent; it exists solely because of the licence.
...
9. **Never add an AGPL dependency — or any dependency outside the [ADR 0004](docs/adr/0004-split-licensing.md) allowlist — under `host/web/`.** The npm allowlist is MIT, ISC, 0BSD, BSD-2-Clause, BSD-3-Clause, Apache-2.0, CC0-1.0, Unlicense, BlueOak-1.0.0, Python-2.0, Zlib and CC-BY-4.0 and **nothing else**; AGPL-3.0 is forbidden outright (audioMotion-analyzer is the named do-not-use — a served web application is exactly the case its network clause reaches), and GPL/LGPL packages are refused too so the allowlist stays one line a reviewer can check. The **CI licence gate over `package-lock.json`** fails closed on an unknown or absent licence, and `npm ci` against the committed lockfile is the only install path ([ADR 0021](docs/adr/0021-host-web-application.md) decision 4).
10. **Never stream live audio or spectra between the watch and the host, in either direction, by any transport.** The contract is **files** — takes in, reports out, presets shared. A live link would need the radio [ADR 0017](docs/adr/0017-no-radio-in-v1-trimmed-component-set.md) excludes, and it is the very thing the research question's prohibition forbids ([ADR 0002](docs/adr/0002-companion-architecture.md) decision 4 as amended by [ADR 0021](docs/adr/0021-host-web-application.md) decision 5).

### Architectural tenets (proposal §3, to be confirmed by ADR 0002/0003/0006)

1. **All real-time DSP of the wearable claim stays on the watch.** The host never sees **the watch's** live audio; it sees takes. Latency and refresh are therefore properties of the firmware alone. The host web application analyses the host machine's *own* microphone and claims nothing about the watch — its latency and refresh are measured, never claimed ([ADR 0021](docs/adr/0021-host-web-application.md)).
...
- Make small, semantically meaningful, subsystem-prefixed commits (`docs: …`, `devenv: …`, `firmware: …`, `web: …`, `ADR(0003): …`). The prefix is a *type* and ADR numbers are its *scope*, so the `conventional-precommit-linter` commit-msg hook (installed 2026-08-21, types listed in `.pre-commit-config.yaml`) can check it — `ADR 0003:` with a space cannot be a type token. Avoid "WIP" commits to `main`.
...
- **Architecture decisions:** [`docs/adr/`](docs/adr/) — toolchain [0001](docs/adr/0001-toolchain-esp-idf-v6-pinned-environment.md), companion split [0002](docs/adr/0002-companion-architecture.md), mic path [0003](docs/adr/0003-microphone-path.md), split licensing [0004](docs/adr/0004-split-licensing.md), anti-brick [0015](docs/adr/0015-anti-brick-policy.md) and the host web application [0021](docs/adr/0021-host-web-application.md) are written and accepted; the [index](docs/adr/README.md) backlog is now 0007 (display path), 0008 (ring/twang metric) and 0020 (f0 estimator)

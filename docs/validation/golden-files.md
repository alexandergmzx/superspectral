...
| **Browser (TypeScript) magnitude spectrum vs the Python oracle** (same window, same normalization per ADR 0006) | per-bin level in dB | `atol = 0.01 dB` for bins ≥ −80 dBFS; bins below are masked | [ADR 0021](../adr/0021-host-web-application.md) D2: the TypeScript module is an *independent* implementation of the ADR 0006 conventions — its own window build, its own FFT kernel, IEEE double throughout — so it is held to the oracle, never to `spectral_core`. The limit starts at the C row's number because that is the budget the conventions were written to, not because the residual is known: **the measured residual is recorded at W1** ([roadmap](../roadmap/documentation-roadmap.md) track W) and this row is tightened **for the TypeScript instrument only** with a recorded reason, never widened to make a run pass |
| **Browser window table digest** per `(family, N)` | sha256 of the N float32 little-endian samples of the periodic table | **exact** | [ADR 0021](../adr/0021-host-web-application.md) D2: the float64 table is rounded **once** to float32 so the digest is the device's digest. Exactness is free here — the coefficients come from the same preset file both sides read — and it is the one row that catches a window rebuilt by name instead of from coefficients. Same reasoning as the `spectral_core` digest row; the measured agreement is confirmed at W1 and there is nothing to tighten |
| **Browser f0 vs the Praat golden**, injection path | median \|Δcents\| over frames voiced in both | ≤ 5 cents `(prov.)` | [ADR 0021](../adr/0021-host-web-application.md) D2, and its **own** row: the browser's estimator is not the watch's ([roadmap](../roadmap/documentation-roadmap.md) Q48 — they may differ, and both are compared to Praat rather than to each other), so merging it with the device row would average away whichever one is wrong. **The measured residual is recorded at W3**; tightened for the TypeScript instrument only, with a recorded reason |
...
- **Browser (TypeScript, GPL side):** `host/web/`'s **vitest golden suite** reads the *committed* `.npy` arrays through a small `.npy` reader — header parse, `descr`/`fortran_order`/`shape` checked, `<f8` and `<f4` only, anything else refused rather than guessed — and applies the tolerance table above; the manifest is read as YAML from the same set, so the digests and the parameters come from the file the Python side generated, not from a copy. There is deliberately **no TypeScript-vs-C row**: the Python oracle is the hub of a star, every implementation is compared to it and never to another, and a browser-vs-device row would turn the star into a clique whose failures name no culprit ([ADR 0021](../adr/0021-host-web-application.md) D2).

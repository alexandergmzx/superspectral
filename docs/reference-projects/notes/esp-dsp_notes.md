...
Against the `fc32` path, sc16 buys roughly **6×** on the S3 (1024 complex points: 15 623 vs 97 847 cycles) at the cost of `log2(N)` bits. For a singing-voice spectrogram whose whole point is a 60–70 dB display range with an in-band SNR of 61.5 dB(A) at the microphone, that trade is not available: **`fc32` only, as [ADR 0006](../../adr/README.md) already pre-registers.** Revisiting sc16 requires a block-floating-point layer (per-stage headroom detection + exponent bookkeeping) that esp-dsp does not provide and that would have to be written and validated against the float path.

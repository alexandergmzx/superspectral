...
- **Studied commit:** `3c8ac0fdfec83740b783e200862c8d0c056de0ad` (master, 2026-05-12, "Merge branch 'bugfix/dp_int8' into 'master'"). The shallow clone carries no tags, but this commit's `CHANGELOG.md` opens with `## [1.8.2] 2026-05-11 — Fixed: Row dot product calculation for int8`, and the merged branch is that fix, so `3c8ac0f` **is** the v1.8.2 content. The clone's `idf_component.yml` has no `version:` key (the release procedure in `CONTRIBUTING.md` adds it on the release branch); **the authoritative record is the pinned managed component**, `firmware/idf-gate/managed_components/espressif__esp-dsp/idf_component.yml`, which carries `version: 1.8.2` and `repository_info.commit_sha: a53a0756833c045311ea1d79a2badf495cdfde4c` — a *different* sha, because the release is tagged on the release branch. The two trees are identical over `modules/` (`diff -rq` reports only the four `test_sim/Makefile`s that the manifest's `files.exclude` drops) and their `CHANGELOG.md` are byte-identical, so everything below reads the same on both. Clone: `docs/reference-projects/clones/esp-dsp/` (gitignored).
...
| Extra allocation | copies the const bit-reversal table into RAM: `malloc(4 × dsps_fft2r_rev_tables_fc32_size[log2(N)−4])`, then **overwrites the global `dsps_fft2r_rev_tables_fc32[log2(N)−4]`** to point at the copy. Only for `16 ≤ table_size ≤ 4096` | same, `dsps_fft4r_rev_tables_fc32[(log2(N)>>1)−2]`, but the guard is **not** the same: fft2r tests `pow > 3 && pow < 13` on `pow = log2(table_size)` (⇒ `16 ≤ table_size ≤ 4096`), fft4r tests `pow >= 2 && pow <= 6` on `pow = log2(max_fft_size) >> 1` (⇒ **`16 ≤ max ≤ 8192`**). Only reachable above 4096 if `CONFIG_DSP_MAX_FFT_SIZE` is raised, which the Kconfig choice does offer (8192/16384/32768) |
...
**A table generated for M works for any FFT of N ≤ M; a table smaller than N is a silent buffer over-read and wrong output.** *(verified: esp-dsp's ANSI `dsps_gen_w_r2_fc32` + `dsps_bit_rev_fc32_ansi` + `dsps_fft2r_fc32_ansi_` compiled standalone; N = 1024 with M ∈ {1024, 2048, 4096} all put the tone at bin 7 with |X| = 512 and peak/total = 0.5, while M = 512 moves the peak to bin 1017 and M = 256 to bin 519, with no error returned.)* Six of the eight init call sites under `examples/` + `applications/` call `dsps_fft2r_init_fc32(NULL, CONFIG_DSP_MAX_FFT_SIZE)`; the two that do not are in `examples/fft4real/main/dsps_fft4real_main.c` — the very example §3 is modelled on — which inits both families at `N >> 1` with `N = 2048`. The over-read experiment above, not the example count, is what settles it: **initialise once, at boot, with the maximum**, because any later library call (`dsps_snr_f32` does exactly this) will otherwise be capped by whatever size we registered first.
...
- The symbol is resolved by the **ROM linker script**, not by esp-dsp: `dsps_fft2r_w_table_fc32_1024 = 0x3fcefff8;` under `Group esp-dsp` in `components/esp_rom/esp32s3/ld/esp32s3.rom.ld` (ESP-IDF v6.0.2). It is a *data* symbol — a 4-byte pointer **variable** in the ROM's DRAM data area, so the declaration as `float *` is type-correct. Read from the ROM ELF shipped with the pinned toolchain (`~/esp/tools/v6.0.2/tools/esp-rom-elfs/20241011/esp32s3_rev0_rom.elf`): `readelf -sW` gives `3fcefff8  4 OBJECT GLOBAL dsps_fft2r_w_table_fc32_1024` in section `[13] .data.interface.esp-dsp` (`PROGBITS 3fcefff8 … W`), inside DRAM (`SOC_DRAM_LOW 0x3FC88000` … `SOC_DRAM_HIGH 0x3FD00000`, `soc/esp32s3/include/soc/soc.h`), in the part the app's `dram0_0_seg` excludes as "memory reserved for ROM bss/data/stack" (`esp_system/ld/esp32s3/memory.ld.in`).
...
- **What it points at is ROM, not RAM.** Its initial value in the ELF is `0x3ff1c050`, which is `dsps_fft2r_w_table_fc32_1024_data` — `4096 OBJECT GLOBAL`, i.e. 1 024 floats — in section `[95] .rodata` (`PROGBITS 3ff18c00 … A`, alloc, **not** writable), the S3's internal-ROM data window above `SOC_DRAM_HIGH`. Dumping those 4 096 bytes gives `1, 0, −0, 1, 0.707107, 0.707107, −0.707107, 0.707107, …` — the already-generated, already-**bit-reversed** twiddle table for `table_size = 1024`, exactly what `dsps_gen_w_r2_fc32` + `dsps_bit_rev_fc32_ansi` would produce.
- Which makes the next step of `dsps_fft2r_init_fc32` a **write into ROM**: having aimed `dsps_fft_w_table_fc32` at `0x3ff1c050`, it calls `dsps_gen_w_r2_fc32(…, table_size)` and then `dsps_bit_rev_fc32_ansi(…, table_size >> 1)` on it unconditionally. At `table_size = 1024` the values it tries to write are the ones already there, so a dropped write is harmless; at `table_size < 1024` it would be writing a *different*, smaller table. Whether the S3 silently discards those stores or raises `StoreProhibited` is **`(prov.)`** — not answerable from the ELF, and settled by a five-line on-target probe (`dsps_fft2r_init_fc32(NULL, 512)` on the S3, read back `((float *)0x3ff1c050)[2]`); it is a `spectral_fft_backend` bring-up item, owned by whoever writes the E-track first-boot test. Either way the note's rule stands and is *strengthened*: this path is one we never take (§4.2 — always pass our own buffer). Two components that both init at ≤ 1024 would also share one physical table.
...
| `dsps_bit_rev_lookup_fc32_{ansi,ae32,aes3}` | the swap engine | table entries are `uint16_t` byte offsets (`>> 2` to a float index in the ANSI version; added to the base pointer unshifted in the aes3 one). The aes3 file is **PIE**: its whole loop body is `l16ui` × 2, `addi`, `add.n` × 2, then `EE.LDF.64.IP` × 2 / `EE.STF.64.IP` × 2 — the same 64-bit FP load/store pair as the FFT kernels, so it inherits their 8-byte alignment requirement (§4.3) |
...
| 2048 | 1024 | **yes** (4⁵) | fft4r + `bit_rev4r` |
...
> **Superseded by [ADR 0006](../../adr/0006-fft-normalisation-and-window-conventions.md) decision 6 (accepted 2026-08-21):** `fft2r` for **every** size in v1. This table is why — two of six presets could use radix-4, at the cost of a second code path, a second set of globals and a second alignment rule — but the decision is the ADR's, not this note's. The revisit trigger is an on-target cycle count (roadmap **H13**). Read the "Path" column below as *what esp-dsp permits*, not as what `spectral_fft_backend` does.

## 4. Memory and alignment

### 4.1 Where the bytes go

`fc32` real-input pipeline, in place, everything in bytes. Costed **under the §4.2 rule** — we always pass our own buffers — so the S3 ROM shortcut for `N_c ≤ 1024` is *not* taken and the real-2048 twiddle row is 4 096 B rather than 0. (If esp-dsp allocated instead, i.e. `NULL`, that row would be 0 and the column total ≈ 35.8 KB. [ADR 0006](../../adr/0006-fft-normalisation-and-window-conventions.md)'s own table books that row as "**0** (const ROM)" — it is a *tables-only* table, so its ≈ 6 KB for real-2048 and this ≈ 39.8 KB are counting different things, and its 0 is the `NULL` route this note tells us not to take.)
...
| fft2r twiddles | `4·N_c` | 4 096 | 8 192 | 16 384 |
...
| **Total** | | **≈ 39.8 KB** | **≈ 77.8 KB** | **≈ 159.8 KB** |
...
- **Write our own `cplx2real`.** It needs only `cos/sin(πk/N_c)` for `k = 0 … N_c/2`, i.e. `4·(N_c+2)` bytes (16 392 B at `N_c = 4096`) instead of 65 536, and it removes the `dsps_fft4r_init_fc32` dependency from the radix-2 path entirely. **On the fft2r path** — 32 768 work + 32 768 window + 16 384 fft2r twiddles + 8 064 fft2r bit-rev copy + 16 392 own half-bin table = 106 376 B — the total for real-8192 drops to **≈ 104 KB**. The saving is *conditional on the radix*: on the fft4r path the 65 536 B radix-4 twiddle table is what the **transform itself** reads (`dsps_fft4r_fc32_ansi_` opens with `if (0 == dsps_fft4r_initialized) return ESP_ERR_DSP_UNINITIALIZED;`), so writing our own `cplx2real` saves nothing there — a pure fft4r real-8192 (no fft2r rows at all) is 32 768 work + 32 768 window + 65 536 fft4r twiddles + 8 064 fft4r bit-rev copy = 139 136 B ≈ **136 KB**, with esp-dsp's `cplx2real` already paid for. This is one of the reasons **[ADR 0006](../../adr/0006-fft-normalisation-and-window-conventions.md) decision 6 settled on `fft2r` for every size in v1**, which supersedes the radix recommendation in §3 and §12 below; the ≈ 104 KB figure carried by [proposal §3](../../proposal/01-super-spectral-proposal.md), [ADR 0018](../../adr/0018-first-reference-project-study.md) and [`docs/architecture/03-dsp-pipeline.md`](../../architecture/03-dsp-pipeline.md) is the fft2r one. (The fft2r twiddle table cannot be reused: it holds `cos/sin(2πi/N_c)` and is stored bit-reversed; `cplx2real` wants half-bin angles in natural order.)
...
A twiddle table in PSRAM violates architecture tenet 3 ("FFT working buffers are internal") and costs on every butterfly. **Therefore `spectral_fft_backend` must always pass its own buffers** to `dsps_fft2r_init_fc32` / `dsps_fft4r_init_fc32`, allocated with `heap_caps_aligned_alloc(16, n, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)` — never `NULL`. (Passing a buffer also skips the S3 ROM shortcut. That is not a loss but a second reason to do it: the shortcut aims the global at a **read-only** `.rodata` table in ROM and then writes to it — §2.3.)
...
| `dsps_bit_rev_lookup_fc32_aes3` | `EE.LDF.64.IP` / `EE.STF.64.IP` only (PIE) | **8 bytes** | **no** |
...
- **Against SciPy the coefficients agree but two of the *names* are swapped.** Checked against `scipy.signal.windows._windows` (scipy 1.18.0): `blackman` `[0.42, 0.50, 0.08]`, `blackmanharris` `[0.35875, 0.48829, 0.14128, 0.01168]` and `flattop` `[0.21557895, 0.41663158, 0.277263158, 0.083578947, 0.006947368]` match esp-dsp coefficient-for-coefficient. But SciPy's **`nuttall`** is `[0.3635819, 0.4891775, 0.1365995, 0.0106411]` — which is esp-dsp's **`blackman_nuttall`**; esp-dsp's **`nuttall`** `[0.355768, 0.487396, 0.144232, 0.012604]` (Nuttall 1981's minimum-4-term-with-continuous-first-derivative) has no SciPy name at all. So "pin Nuttall's coefficients" ([bibliography 05 #3](../../bibliography/05-papers.md)) means pinning *which set under which name*, not just the numbers. [ADR 0006](../../adr/0006-fft-normalisation-and-window-conventions.md) records the swap.
- **Praat is not a reference for this table.** Its window shapes are `square/Hamming/Bartlett/Welch/Hanning/Gaussian` (`praat/fon/Sound_and_Spectrogram_enums.h`, `kSound_to_Spectrogram_windowShape`) and `rectangular/triangular/parabolic/Hanning/Hamming/Gaussian1–5/Kaiser1–2` (`praat/fon/Sound_enums.h`, `kSound_windowShape`) — read from the vendored source in `docs/reference-projects/clones/Parselmouth/`. No Blackman, no Blackman–Harris, no Nuttall, no flat-top: five of esp-dsp's six windows have no Praat counterpart to agree with. (The hits for "Blackman"/"Nuttall" under `praat/` are all inside the vendored FLAC and LAME encoders.)
- The remaining discrepancy against a periodic reference is the `1/(len−1)` denominator, which applies to all six.
...
- (−) **Not** the radix table in §3. It reads fft4r for real-2048 and real-8192, fft2r for real-1024 and real-4096 — but [ADR 0006](../../adr/0006-fft-normalisation-and-window-conventions.md) decision 6 chose **`fft2r` for every size in v1**: two of six presets do not justify a second code path, a second set of globals and a second alignment rule, and the default analysis size (real-4096) cannot use radix-4 at all. The backend-agreement test therefore covers one kernel in v1; carrying both is deferred to the roadmap **H13** cycle count, which is the ADR's revisit trigger.
- (≈) `dsps_cplx2reC_fc32` as a **two-for-one**: two real frames of `L` samples (two overlapping analysis frames, or two channels) through **one complex FFT of `L` points**, one packed into re and one into im (`dsps_fft2r.h`: "input has size of 2\*N … Input1: input[0..N-1], Input2: input[N..2\*N-1]", `N` = complex elements). **It does not halve anything against our baseline.** §3 already does one complex FFT of `L/2` per frame, so per two frames the comparison is `1 × FFT(L)` against `2 × FFT(L/2)` — `a·L·log2 L` against `a·L·(log2 L − 1)`, one extra radix-2 stage for the trick. On §11's own numbers, two real-1024 frames cost 2 × 44 594 = **89 188** cycles the §3 way and 1 × 97 847 = **97 847** cycles the `cplx2reC` way, i.e. **9.7 % more**, plus a 2× larger complex buffer and the packing loop. `cplx2reC` halves cost only against two *full-length* complex FFTs of real input, which is the [xiao-edge-audio](xiao-edge-audio_notes.md) baseline, not ours. Its remaining merit is one call and one set of globals for two channels — which v1 does not have (one PDM mic, [ADR 0003](../../adr/0003-microphone-path.md)).
...
|---|---|---|
| [`spectral_fft_backend` README](../../../firmware/twatch-s3/components/spectral_fft_backend/README.md) | work area "~32 KB for n = 4096 (prov.)" | the real-4096 work area is ≈ 78 KB with esp-dsp's own tables, **≈ 52 KB** with our own `cplx2real` — 16 384 work + 16 384 window + 8 192 fft2r twiddles + 3 968 fft2r bit-rev copy + 8 200 own half-bin table = 53 128 B. (Its `fft4real` half is **already fixed**: the README now reads "there is no `fft4real` API".) |
...
| [ADR 0006 backlog entry](../../adr/README.md) | "float32 `fc32` only — no `sc16` without a block-floating-point layer" | confirmed, with the arithmetic: fixed `>>16` per butterfly = 1/2 per stage = 1/N overall, `log2(N)` bits lost, no exponent tracking (§8) — **closed by [ADR 0006](../../adr/0006-fft-normalisation-and-window-conventions.md) decision 4** |

**Closed since this note was written** (re-read 2026-08-21; kept as a record, not as a checklist):

| Document | Was | Now reads |
|---|---|---|
| [bibliography 06 #1](../../bibliography/06-reference-projects.md) | "`dsps_fft2r/fft4r/fft4real`" | "there is **no** `fft4real` API, `examples/fft4real/` is a directory and the real path is composed by hand" — with the `dsps_fft4r_init_fc32` dependency spelled out |
| `firmware/twatch-s3/main/idf_component.yml` comment | "radix-2/4 complex FFT, fft4real, six windows" | "radix-2/4 complex FFT (no real-FFT entry point — the real path is composed by hand), six **symmetric** windows" |
| [bibliography 02 #21](../../bibliography/02-application-notes.md) | "real-8192 fits 112 KB SRAM" | "real-8192 fits ≈ 112–144 KB `(prov.)` SRAM". The pair this note supports is **≈ 104 KB** with our own `cplx2real` on fft2r against **≈ 160 KB** with esp-dsp's tables, 64 KB of which lands in PSRAM by default |

Reference basis: the pinned managed component `firmware/idf-gate/managed_components/espressif__esp-dsp/` (`version: 1.8.2`, `commit_sha: a53a0756`) as the record, and the study clone at `3c8ac0f` — tree-identical to it over `modules/` — for `modules/fft/{float,fixed,include}`, `modules/windows/*`, `modules/iir/*`, `modules/support/{snr,mem,misc,cplx_gen}`, `modules/fir/*`, `modules/common/{include,misc}`, `Kconfig`, `CMakeLists.txt`, `LICENSE`, `CONTRIBUTING.md`, `docs/en/esp-dsp-benchmarks.rst`, `test/`, `test_app/`, `examples/{fft,fft4real}`, `applications/spectrum_box_lite`; ESP-IDF v6.0.2 sources for `dsps_fft2r_w_table_fc32_1024` (`components/esp_rom/esp32s3/ld/esp32s3.rom.ld`, `components/soc/esp32s3/include/soc/soc.h`) and the ROM ELF `esp-rom-elfs/20241011/esp32s3_rev0_rom.elf` from the pinned tool set (symbol table, section flags and the 4 096 B `.rodata` payload behind that pointer), the DRAM/ROM split (`components/esp_system/ld/esp32s3/memory.ld.in`), and the allocator routing (`components/esp_libc/src/heap.c`, `components/heap/heap_caps.c`, `components/esp_psram/Kconfig.spiram.common`); [bibliography 06 #1](../../bibliography/06-reference-projects.md) (the project), [02 #20](../../bibliography/02-application-notes.md) (ESP-DSP API reference) and [02 #21](../../bibliography/02-application-notes.md) (ESP-DSP benchmarks); [04 #1](../../bibliography/04-books.md) (Smith, *SASP* — the real-FFT N/2 packing identity), [05 #1](../../bibliography/05-papers.md) (Heinzel 2002 — S1/S2, NENBW, periodic windows), [05 #2](../../bibliography/05-papers.md) (Harris 1978) and [05 #3](../../bibliography/05-papers.md) (Nuttall 1981) for the window table, cross-checked against `scipy.signal.windows._windows` (scipy 1.18.0) and the vendored Praat window enums in `docs/reference-projects/clones/Parselmouth/praat/fon/`; [ADR 0017](../../adr/0017-no-radio-in-v1-trimmed-component-set.md) for the analyzer policy that produced the two findings; [`docs/hw/README.md`](../../hw/README.md) for the measured chip revision, clock and PSRAM facts of this unit (2026-08-20); [`firmware/twatch-s3/sdkconfig.defaults.esp32s3`](../../../firmware/twatch-s3/sdkconfig.defaults.esp32s3) for the PSRAM allocator thresholds this note reasons against.

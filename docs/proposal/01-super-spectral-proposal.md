...
| Sample rates | 16 kHz (`DSR_16S`, 2.048 MHz) · **32 kHz default** (`DSR_8S`, 2.048 MHz) · 48 kHz needs 3.072 MHz, **5.8 % under the mic's 3.25 MHz maximum** — gated on measurement (threshold T3) | 01 #9, 02 #14 |
...
### 3.2 Companion split (normative; ADR 0002, pre-registered in the ADR backlog)
...
| CPP / CPPS, AVQI-class composites | ✗ | ✅ | acoustic material only; no clinical claim (ADR 0005, backlog) |
...
Licensing follows the same line: Apache-2.0 for the repository, firmware, documentation and tooling; `host/` **GPL-3.0-or-later** with its own `LICENSE` so it may import parselmouth/Praat in-process; no code crosses the boundary in either direction, only files on disk (ADR 0004, backlog; 03 #36, 03 #37, `NOTICE`).
...
- **Tier 3 — pathology:** Saarbrücken (10 #19) and PVQD, **acoustic material only, no clinical claim** (ADR 0005, backlog; 03 #32, 03 #33, 03 #34).
...
- **Native-Linux simulator target** (ADR 0013, backlog): the LVGL simulator plus the DSP core compiled for the host, giving the UI design loop and the golden-file harness in one target (11 #A36).

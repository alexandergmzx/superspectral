...
- **Definition of done**
  - [x] The `host` job green **including `spectral-golden verify`** — the golden pins do not move because a web application appeared.
  - [x] The `web` job green: `npm ci` against the committed lockfile, licence gate, `tsc --noEmit`, vitest.
  - [x] `GET /api/presets/{id}` returns bytes **identical** to `protocols/presets/<id>.json` for all six presets (sha256 compared in the test and displayed beside the preset name), and a deliberately tampered preset **fails loudly with rule V0** of the loader rather than being served.
  - [x] `spectral-web peak` reports the peak within **≤ 3 cents** on **both** `sine_1000_m20dBFS_32k.wav` *and* `sine_440_m20dBFS_32k.wav` (the peak-frequency row of [`../validation/README.md`](../validation/README.md)). The 1 kHz file alone does not test the estimator: at `fs` = 32 kHz and `N` = 4096 it lands **exactly** on bin 128, so a stubbed interpolator returning `0.0` passes it. 440 Hz falls at bin 56.32 — the raw bin centre is 9.87 cents out, so only interpolation brings it inside the row. *(Row corrected 2026-08-22 from the W0 review, which found the original satisfiable by a no-op.)*
  - [x] A deliberately added **AGPL-3.0** package **fails** the licence gate; the negative test and its exact output are recorded (ADR 0021 decision 4 names audioMotion-analyzer as the do-not-use).
  - [x] The licence-boundary greps still clean: every file under `host/` carries the GPL-3.0-or-later header or a `host/REUSE.toml` row, and no file outside it does ([ADR 0004](../adr/0004-split-licensing.md)).

  *Closed 2026-08-22 by commit `5867e2a`. Evidence: CI run [32561450895](https://github.com/alexandergmzx/superspectral/actions/runs/32561450895) on `5867e2a` — **all ten jobs green, the `web` job's first remote run** (47 packages installed, 116 scanned by the licence gate, 2 test files passed, `dist/` built). Locally: host 423 passed with zero skips; `spectral-golden verify` ok (14 rules) with the seven `GENERATOR_TREE` files byte-identical to HEAD; `spectral-web peak` +0.00 cents at 1 kHz and **+0.48 cents at 440 Hz** (raw bin centre 9.87 cents out); the AGPL-3.0 negative test refused two packages and exited 1; all six presets `cmp`-identical over the wire. Open for the owner: Vite is pinned `^7` because Vite 8 pulls `lightningcss` (MPL-2.0), which is not on the ADR 0021 decision 4 allowlist — admitting MPL-2.0 is a policy change and his signature.*
...
| T7b | **Bundled Praat 6.1.38 vs praat.org 7.0.01** (one WAV through both — *version* drift and *method* drift measured separately) | outputs differ beyond the tolerance table | Golden files already pin the bundled version and method explicitly; the "≡ Praat" claim is dropped from the proposal and the RQ names the version it means; tolerance table widened only with a recorded reason. **Open** — needs a praat.org binary run out of process, and Praat 7.0's script full-trust checking handled for a script that writes files (`--FULL-TRUST`, `sys/praat.cpp`). Recipe pre-registered 2026-08-22: [experiment 0005](../validation/experiments/0005-t7b-bundled-praat-vs-praat-org.md). | golden-files.md manifest; ADR 0009; experiment 0005 |

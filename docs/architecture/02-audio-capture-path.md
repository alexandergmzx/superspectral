...
1. **`I2S_PDM_RX_SLOT_PCM_FMT_DEFAULT_CONFIG`, not `I2S_PDM_RX_SLOT_DEFAULT_CONFIG`** — the latter survives in v6.0.2 only as a `@cond` alias. On a `SOC_I2S_SUPPORTS_PDM2PCM` target the macro expands **without** `hp_en` / `hp_cut_off_freq_hz` / `amplify_num`; those fields do not exist on the S3, which is why §7's blocker is not optional.
...
### 10.2 The rail — now half-answered
...
The *rail-identity* half is still open (roadmap **Q14 / H2 / H-rail**, closed by the schematic in D3/D4) but has strong code evidence short of it: LilyGoLib's `RecordWAV` example records from the microphone **before** it calls `powerControl(POWER_SPEAK, true)`, `initPMU()` has already executed `disableDLDO1()`, and TTGO's branch carries `enableDLDO1(); //! Speaker` — two independent witnesses that **DLDO1 is the amplifier rail** and that the **microphone sits on a rail that is on before any firmware runs**, i.e. the always-on 3.3 V system rail *(prov.)* ([study notes](../reference-projects/notes/lilygolib-axp2101_notes.md) §6.3).

Three consequences for this component, all of which hold either way: the firmware does **not** attempt to power-gate the microphone; the capture-path term of the power budget is a `TBD` against the ≥ 3 h autonomy bound (`06-power-budget.md`); and I2S0 is enabled only after the `twatch_bsp` PMU bring-up sequence has settled. One risk this reading removes: disabling DLDO1 at boot is safe and does **not** silence the microphone.
...
| 2 | DC blocker **form** — first-order blocker vs RBJ biquad — and the host/device tolerance it must meet | ADR 0003 d.6 permits both; the [esp-dsp notes](../reference-projects/notes/esp-dsp_notes.md) §6 call it a `spectral_core` decision | **ADR 0006** (pre-registered, [ADR backlog](../adr/README.md)) |
...
| 5 | PDM slot mask | three contradicting vendor artefacts (§10.1) | roadmap **H7** / H-slot — E2 first capture |
| 6 | Which AXP2101 rail powers the SPM1423 (and the MAX98357A) | code evidence, not a schematic reading (§10.2) | roadmap **Q14 / H2 / H-rail** — D3/D4 |

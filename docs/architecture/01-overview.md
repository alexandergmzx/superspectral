...
| 3 | **FFT normalisation and window conventions are not yet a single-source spec.** ADR 0006 is pre-registered and unwritten; the watch↔host agreement tolerance depends on it | [ADR backlog 0006](../adr/README.md); [`../../dsp/design/`](../../dsp/design/README.md) |
...
| 6 | **The capture-path power term is `TBD`.** Which AXP2101 rail powers the SPM1423 (and the MAX98357A) is unresolved — strong code evidence, no schematic — so the microphone cannot be power-gated and the ≥ 3 h autonomy bound carries an unbudgeted term | roadmap Q14 / H2; `06-power-budget.md` (planned, [README](README.md)) |

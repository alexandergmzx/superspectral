...
┌─ LINUX HOST — OFFLINE PATH + ITS OWN UI (host/, GPL-3.0-or-later) ───────────────┐
...
│  host web application (host/web/ + src/spectral_host/web/, ADR 0021): a live     │
│    analyzer of THIS machine's microphone · injection mode · offline-compare UI   │
│    its latency and refresh are MEASURED, never claimed — no wearable bound       │
│  Never in the watch's live loop · no live link at all · not linked into firmware │
└──────────────────────────────────────────────────────────────────────────────────┘
```
...
| The offline depth the wrist cannot honestly provide (metrics needing an SPL co-report, formant bandwidths, or a whole take) | Never in **the watch's** live loop. It never sees the watch's live audio, only takes — so no host latency ever enters a research-question number ([ADR 0002](../adr/0002-companion-architecture.md) decision 3 as amended by [ADR 0021](../adr/0021-host-web-application.md) decision 5) |
...
| A file consumer: takes in, reports and golden vectors out | Not an RPC peer, a streaming sink, or a live viewer **of the watch** — there is no live link between the two ([ADR 0002](../adr/0002-companion-architecture.md) decision 4 as amended by [ADR 0021](../adr/0021-host-web-application.md)), and no second transport. The live view it *does* have is of **its own microphone** |
| The **host web application** ([ADR 0021](../adr/0021-host-web-application.md)): a browser-native live analyzer of the host's own microphone, an injection mode that replays a Tier-0 or corpus WAV through the same TypeScript DSP module, and the offline-compare user interface the `analyze/` and `compare/` modules never had | **Not a wearable claim.** Its mic-to-pixel latency and its sustained refresh are **measured and reported per browser and OS, never claimed** — they carry no bound of [proposal §1](../proposal/01-super-spectral-proposal.md), and no number it produces may be quoted for one (ADR 0021 decision 3) |
...
**C — the injection loop (how the two halves are compared).** The same corpus WAV is replayed through `file_blob` into the PCM ring with the microphone bypassed, and analysed by the host from the identical file. Bit-exact `PCM_BLOCK`s are what make "the host recomputes what the watch displayed" testable rather than rhetorical. This is the only path on which a "vs Praat" claim is legitimate ([`../validation/README.md`](../validation/README.md), two-path rule). Since [ADR 0021](../adr/0021-host-web-application.md) this loop has **two instruments, not one**: the host web application's injection mode replays the same Tier-0 files through an independent TypeScript implementation of the [ADR 0006](../adr/0006-fft-normalisation-and-window-conventions.md) conventions against the same golden set, so a convention bug in `spectral_core` or in the Python oracle no longer has only one witness — and because every implementation is compared to the oracle and never to another, the agreement matrix stays a star (there is no watch-vs-browser row).
...
| ~~3~~ | ~~**FFT normalisation and window conventions are now specified but not ratified.**~~ **Closed 2026-08-21.** ADR 0006 **accepted** (nine decisions D1–D9, four of them corrected by an independent audit before acceptance); the watch↔host agreement tolerance now rests on an accepted record | [ADR 0006](../adr/0006-fft-normalisation-and-window-conventions.md); [`../../dsp/design/`](../../dsp/design/README.md) |
...
| 9 | **The web application's sample-rate rule has an unresolved tail.** The presets state 32 kHz and browsers default to 44.1 or 48 kHz. [ADR 0021](../adr/0021-host-web-application.md) decision 7(c) settles the rule — construct `new AudioContext({sampleRate: preset.analysis.sample_rate_hz})`, **assert** the resulting `AudioContext.sampleRate`, and **refuse to start** on a mismatch naming both rates, because a silent browser resampler would make `bin_width_hz` and `enbw_hz` fiction; injection mode sidesteps the question entirely by parsing int16 PCM itself instead of calling `decodeAudioData`. What stays open is only the tail: **what to do if a target browser cannot honour 32 kHz at all** — refuse that browser, add a declared linear resampler ([ADR 0003](../adr/0003-microphone-path.md) decision 7's rule), or carry a 48 kHz live preset. None of the three is chosen | [ADR 0021](../adr/0021-host-web-application.md) decision 7(c); [roadmap](../roadmap/documentation-roadmap.md) track W (W1/W2) |

---

Reference basis: [ADR 0002](../adr/0002-companion-architecture.md) for the split, the normative feature table, the take-format constraints and the T2 pivot; [ADR 0003](../adr/0003-microphone-path.md) for the capture path (I2S0-only PDM RX, 16-bit slots, 32 kHz/`DSR_8S`, software DC removal, no NS/AGC/AEC), grounded in `driver/i2s_pdm.h` + `i2s_pdm.c` ([02 #15](../bibliography/02-application-notes.md)), `soc_caps.h` for esp32s3 ([02 #16](../bibliography/02-application-notes.md)) and the Knowles SPM1423HM4H-B spec ([01 #9](../bibliography/01-datasheets.md)); [ADR 0013](../adr/0013-native-linux-simulator-target.md) for the `audio_source`/`display_backend` seams and the three lanes; [ADR 0009](../adr/0009-golden-file-strategy.md) and parselmouth ([05 #55](../bibliography/05-papers.md)) for the host-as-arbiter role; [ADR 0014](../adr/0014-partition-layout-frozen.md), [0015](../adr/0015-anti-brick-policy.md), [0017](../adr/0017-no-radio-in-v1-trimmed-component-set.md) for the storage, recovery and radio boundaries; [ADR 0004](../adr/0004-split-licensing.md) and [0005](../adr/0005-no-clinical-claim.md) for the licence and claim boundaries; [ADR 0021](../adr/0021-host-web-application.md) for the host web application — the host's own user interface, the second injection-path instrument, the *measured, no claim* rule on its latency and refresh, and the amendment that makes ADR 0002's decision 4 read *no live link between watch and host*; esp-dsp ([06 #1](../bibliography/06-reference-projects.md)) with its published benchmarks ([02 #21](../bibliography/02-application-notes.md)) and our own [esp-dsp study notes](../reference-projects/notes/esp-dsp_notes.md) for the compute and SRAM envelope; MPM ([05 #8](../bibliography/05-papers.md)) for the on-device f0 choice; the founding [research document](../research/00-linux-analyzer-architecture-and-build-guide.md) for the six presets and the live/offline ancestry. Measured on this unit: [`../hw/README.md`](../hw/README.md) (eFuse baseline, flash identity, USB-Serial-JTAG read rate), [`../../firmware/idf-gate/`](../../firmware/idf-gate/README.md) (E1 gate, 2026-08-20) and [experiment 0002](../validation/experiments/0002-rollback-and-boot-guard-race.md) (rollback and boot-guard race, 2026-08-21).

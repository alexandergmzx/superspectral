...
Two independent `i2s_new_channel()` calls with different `i2s_chan_config_t.id`. DMA and ring buffers `MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA` ([pitfalls D5](../devenv/pitfalls.md#d-memory-placement-and-runtime)). PDM slot: LilyGoLib captures one channel only — which `slot_mask` (`I2S_PDM_SLOT_LEFT`/`RIGHT`) carries the SPM1423 is an E2 experiment; the wrong one captures silence and looks like a dead mic. Rates: 16 kHz/`DSR_16S` → 2.048 MHz clock; 32 kHz/`DSR_8S` comfortable; 48 kHz needs 3.072 MHz — 178 kHz of margin, 5.5 % of the 3.25 MHz ceiling (5.8 % if taken relative to the 3.072 MHz clock) — verify (ADR 0003). No hardware PDM high-pass on the S3 (`SOC_I2S_SUPPORTS_PDM_RX_HP_FILTER` absent from `soc_caps.h` in v6.0.x): DC removal is software.
...
| GPIO46 (I²S DIN) — strapping pin | ROM-message printing (with `EFUSE_UART_PRINT_CONTROL` / `EFUSE_DIS_USB_SERIAL_JTAG_ROM_PRINT`) and chip-boot-mode qualifier together with GPIO0; **weak pull-down, bit value 0** at reset *(verified 2026-08-21 — ESP32-S3 datasheet v2.2 §3 Boot Configurations, Table 3-1)*. All four straps are **latched at Chip Reset** and the pins are then freed for ordinary I/O, so only the level *across the reset edge* matters | The MAX98357A input is high-impedance at reset, so the strap sees the pull-down; never add an external pull-up; I2S1 is configured after boot |
| GPIO3 (LoRa SCK) — strapping pin | JTAG signal-source select, with `EFUSE_DIS_PAD_JTAG` / `EFUSE_DIS_USB_JTAG` / `EFUSE_STRAP_JTAG_SEL`; **floating** by default — no internal pull either way, so its bit value is undefined unless a circuit sets it *(verified 2026-08-21 — datasheet v2.2 Table 3-1)*. All three eFuses read 0 on this unit ([`efuse-baseline.json`](efuse-baseline.json)), and CLAUDE.md forbids burning any of them, so the USB-Serial-JTAG path stays selectable | SX1262 held in reset, ALDO4 off (ADR 0017); SPI2 is not initialised in v1 |

...
| D5 | **LilyGO T-Watch S3 case drawing / STEP, or a teardown-measured port drawing** — home: [01](01-datasheets.md) (request to vendor; fallback: calipers + photo) | ★★★ | Hole diameter, channel length, cavity volume, gasket presence. **No vendor file exists for the case body** (bounded search 2026-08-21: LilyGO GitHub org, product page, wiki — the wiki's "Dimension Diagram" is a spec-sheet JPEG with no dimensions; the page text gives 51.5 × 42 × 20 mm without strap). What does exist is two **back-cover** STEP models on the `t-watch-s3` branch of `TTGO_TWatch_Library` (`shell/BackCover.stp`, Pro/ENGINEER 2024-08-27, part `K254`; `shell/BackCover1.stp`, Creo 2025-02-07, `ASM0001_ASM`; MIT, at `9884d62`) — the removable back, not the port-bearing shell, so they bound the cavity at most. Ledgered `n/a`: **calipers + photo → `hardware/acoustic-port/` (owner + hardware)** is the acquisition path, per [`hardware/acoustic-port/README.md`](../../hardware/acoustic-port/README.md). |
...
| D5 | n/a / request | **No vendor case-body file exists** (searched 2026-08-21 — see the row); the only vendor mechanical files are the two back-cover STEPs in the cloned [`TTGO_TWatch_Library` `t-watch-s3` branch](https://github.com/Xinyuan-LilyGO/TTGO_TWatch_Library/tree/t-watch-s3/shell) (`shell/BackCover.stp`, `shell/BackCover1.stp`). Acquisition path: **calipers + photo → [`hardware/acoustic-port/`](../../hardware/acoustic-port/README.md) (owner + hardware)**; a vendor request via [LilyGO support](https://lilygo.cc/pages/contact) *(link replaced 2026-08-21: the slug lost its `-us`; `/pages/contact-us` is 404)* stays open as the long shot |
...
**D5 bounded search, 2026-08-21 (Phase-0 ★★★ closure).** One pass, three places: the `Xinyuan-LilyGO` GitHub organisation through the two cloned repositories (`LilyGoLib` at `38e6f8d` — its `case/` directory holds T-LoRa-Pager STLs only; `TTGO_TWatch_Library` branch `t-watch-s3` at `9884d62` — `shell/` holds `BackCover.stp` 1.62 MB and `BackCover1.stp` 0.77 MB with renders, and the GitHub contents API lists the same five files upstream), the product page `lilygo.cc/products/t-watch-s3` (no mechanical-file link of any kind), and the wiki page (its "Dimension Diagram" heading is `t-watch-s3-info.jpg`, a 1000 × 1000 spec sheet — MCU, radio, display — with no dimension on it; the page body gives 51.5 × 42 × 20 mm). Result: the "no vendor file" claim is **true for the case body and false for the back cover**; the row now says both. The two STEP renders were looked at, not the solids — whether the back cover contributes to the front-cavity volume is a teardown question, not a CAD one. The model-recalled no-public-STEP claim in the two bullets above is thereby replaced by a record.

...
**"host" in the Targets column means both host programs.** Since [ADR 0021](../../docs/adr/0021-host-web-application.md) decision 7(b) the `host` target names the CLI and offline tools *and* the **host web application** under [`../../host/`](../../host/README.md); `targets` itself is unchanged ([ADR 0010](../../docs/adr/0010-preset-schema.md)). The web application **never carries its own copy** of these files and its bundler never embeds one: it fetches each from the backend, byte-identical, after the loader has proved it, and displays the sha256 of exactly those bytes beside the preset name — so drift between the served preset and this directory is a red test, not a fork. The five presets with a `refresh_hz_target` are its **live** presets; `stem_analysis` (host-only, 48 kHz) is its **offline-compare** preset.

...
Human-readable specifications of the formats that cross the watch ↔ host boundary. Implementations live in [`../../firmware/`](../../firmware/) (writer, C99) and [`../../host/`](../../host/) (reader, Python); machine-readable schemas and test vectors live next to this directory under `../schemas/` (planned). The record-kind table in [`../README.md`](../README.md) is the single source of truth for kinds and sizes.

Written:

- *(none yet — formats are specified in roadmap D5/D6 together with ADR 0010 and ADR 0014, before any firmware writes a take)*
...
- `preset-schema.md` — the preset JSON (ADR 0010): every field of the founding document's six presets (`fft_size`, `decimations`, `window`, `interval_ms`, `smoothing`, `overlays`, `freq_scale`, `averaging`, `hold`) plus the fields the watch needs and the original never stated — **explicit analysis bandwidth / ENBW** derived from window and FFT size under the convention in [`../../dsp/design/`](../../dsp/design/), **mic-EQ slot** (`none` | `<eq-id>`), sample rate, display refresh target, colormap id — with a JSON Schema, defaults, and the rule that the watch validates and rejects rather than silently coercing. Does not define DSP conventions; it references `dsp/design/fft-normalization.md`.

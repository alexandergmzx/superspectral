...
This spec does **not** define the DSP conventions. Normalisation (`1/N` vs `1/√N`, S1/S2 sums, power spectrum vs PSD scaling) is [ADR 0006](../../docs/adr/README.md) and `dsp/design/fft-normalization.md`; the decimation cascade is `dsp/design/decimation-cascade.md`; the ring/twang readout is [ADR 0008](../../docs/adr/README.md). See [`../../dsp/design/README.md`](../../dsp/design/README.md).
...
| `smoothing` | number 0–0.95 | exponential coefficient; must be > 0 iff `averaging == "exponential"`, and exactly 0 otherwise. The 0.95 ceiling is Spectroid's documented range |
| `hold` | `none` \| `peak` \| `min` | |
...
| `coefficients` | array of 2–5 numbers | the cosine-sum *a<sub>k</sub>*, exactly as committed in esp-dsp — the Blackman–Harris set is the standard 4-term −92 dB one and the Nuttall set is Nuttall 1981's minimum-4-term, so watch, host, SciPy and Praat agree coefficient-for-coefficient ([`esp-dsp_notes.md`](../../docs/reference-projects/notes/esp-dsp_notes.md) §5) |
...
`scope` records the open question the EQ itself raises — whether the correction is a per-part-number constant or a per-unit calibration. A per-unit EQ makes every band ratio non-reproducible on a second watch without a calibration step. The schema records the answer; it does not choose it (`dsp/design/mic-eq.md`, roadmap Q38).
...
| `refresh_hz_target` | integer 1–50 | **required for a watch preset.** 50 Hz is the ceiling of the hardware-scrolled analyzer canvas; `1000 / interval_ms` must be an integer multiple of it (rule V7), i.e. a whole number of spectrogram columns per rendered frame |
...
| V7 | `interval_ms × sample_rate_hz` is a multiple of 1000; `1000 / interval_ms` is an integer multiple of `refresh_hz_target` | loader |
...
CPU figures are the preset-feasibility estimates of the domain-map study (real N at the stated frame rate, +40 % for window, bit-reversal, `cplx2real`, magnitude and fast-log); "derived" marks a figure scaled from a published rate to this preset's frame rate rather than quoted directly. Every one is re-measured on target in Phase 1 with `dsp_get_cpu_cycle_count()`. Internal-SRAM working set: 56 KB at N = 4096, 112 KB at N = 8192, below the N = 2048 row's 24 KB at N = 1024.
...
|---|---|---|
| Schema validation of `protocols/presets/*.json` | every shipped preset is loadable | `python-scripts/check_presets.py` (planned) + a CI job; run manually against `jsonschema` 4.10.3 `Draft202012Validator` when ADR 0010 was written — 6/6 valid |
| Negative-case suite | the loader *rejects* every malformed preset. 30 cases at the time of writing — 21 caught by the schema (unknown field, off-enum window, N = 16384 on a watch preset, host-only overlay on a watch preset, `smoothing > 0` without exponential averaging, `averaging_frames` without linear averaging, `host` block on a watch-only preset, missing `enbw_hz`, `schema_version` 2.x, log scale with `freq_min_hz` 0, `mic_eq` `ref` without a sha256, …) and 9 only by rules V1/V6b/V7/V8/V9/V10 (mutated coefficients, inconsistent `enbw_bins`, `enbw_hz` or `hop_samples`, a hop giving fractional columns per frame, `id` ≠ filename, an unresolvable `provisional` pointer, `freq_max_hz` above Nyquist, an inline mic EQ designed at another sample rate) | same script; 30/30 rejected, 4/4 positive controls accepted |
| Rules V0–V10 recomputation | the derived constants in the file agree with the coefficients and with `(N, f_s, interval_ms)` to 1×10⁻⁶ | same script; also a `spectral_core` host test (planned), because the C loader must reject the same files |
